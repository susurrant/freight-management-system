...
namespace 运输计划管理系统
{
    public partial class 上报运输周计划列表 : Form
    {
        private string cmp;
        private string year;
        ...
        public 上报运输周计划列表(string usrKind)
        {
        ...
        private void 上报运输周计划列表_Load(object sender, EventArgs e)
        {
        ...
        {
            String qStr = "select * from WeekPlanning where status <> '已下达' and status <> '未下达'";
            if (this.usrKind == "审批员")
            {
                qStr = "select * from WeekPlanning where status <> '已下达' and status <> '未下达' and status <> '新增'";
            }
        ...
        private void btn_approve_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
            {
                if (this.dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "待初审")
                    {
                        if (this.usrKind != "审批员")
                        {
                            MessageBox.Show("用户权限不够！", "提示");
                        ...
                        计划审批 dlg = new 计划审批(this.dataGridView1.Rows[i].Cells["ID"].Value.ToString(), "周");
                        dlg.ShowDialog();
                    }
                    else if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "新增")
                    {
                        MessageBox.Show("该周计划尚未提交！", "错误");
                    }
                    else if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "通过")
                    {
                        MessageBox.Show("该周计划已审批通过！", "禁止");
                    }
                }
            }
        }
            ...
            for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
            {
                if (this.dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "通过" ||
                        this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "不通过")
                    {
                        审批单 dlg = new 审批单(this.dataGridView1.Rows[i].Cells["ID"].Value.ToString(), "周");
                        dlg.ShowDialog();
                    }
                    else
                    {
                        MessageBox.Show("该周计划尚未审批！", "提示");
                    }
                }
