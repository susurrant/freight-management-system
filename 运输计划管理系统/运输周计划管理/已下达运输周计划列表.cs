...
namespace 运输计划管理系统
{
    public partial class 已下达运输周计划列表 : Form
    {
        public 已下达运输周计划列表()
        {
            InitializeComponent();
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            ...
            string zs = this.cb_zs.Text;

            string sj = year + month;
            String qStr = "select company,date,amount,type,ID from WeekPlanning where company " + (cmp != "" ? ("= '" + cmp + "'") : "is not null") +
                    " and date " + (sj != "" ? ("like '" + sj + "%'") : "is not null") + " and type " + (ptype != "" ? (" = '" + ptype + "'") : "is not null") +
                    " and status = '已下达' and week " + (zs != "" ? ("= " + zs) : "> 0");
