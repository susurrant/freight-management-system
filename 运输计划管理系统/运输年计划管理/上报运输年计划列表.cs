...
namespace 运输计划管理系统
{
    public partial class 上报运输年计划列表 : Form
    {
        private string cmp;
        private string year;
        ...
        public 上报运输年计划列表(string usrKind)
        {
        ...
        private void 上报运输年计划列表_Load(object sender, EventArgs e)
        {
        ...
        {
            String qStr = "select * from YearPlanning where status <> '已下达' and status <> '未下达'";
            if (this.usrKind == "审批员")
            {
                qStr = "select * from YearPlanning where status <> '已下达' and status <> '未下达' and status <> '新增'";
            }
        ...
        private void btn_mod_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
            {
                if (this.dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "不通过" ||
                        this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "新增")
                    {
                        if (this.usrKind != "业务员")
                        {
                            MessageBox.Show("用户权限不够！", "提示");
                        ...
                        运输计划修改 dlg = new 运输计划修改(this.dataGridView1.Rows[i].Cells["ID"].Value.ToString(), "年");
                        dlg.ShowDialog();
                    }
                    else
                    {
                        MessageBox.Show("该年计划不可修改！", "提示");
                    }
                }
            }
        }
            ...
            for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
            {
                if (this.dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                        ...
                        this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "不通过")
                    {
                        ...
                        String qStr = "update YearPlanning set status = '待审' where ID = '" + this.dataGridView1.Rows[i].Cells["ID"].Value.ToString() + "'";
                        SQLquery query = new SQLquery(qStr);
                        if (query.doModify())
                        {
                            MessageBox.Show("年计划提交成功！", "提示");
                            updateDataGridView();
                        }
                        else
                        {
                            MessageBox.Show("年计划提交失败！", "提示");
                        }
                    }
                    else
                    {
                        MessageBox.Show("该年计划不可提交！", "提示");
                    }
                }
            }
        }
        ...
        private void updateDataGridView()
        {
            if (this.searchClick == true)
            {
                ...
                String qStr = "select company,date,amount,type,status,ID from YearPlanning where company " + (cmp != "" ? ("= '" + cmp + "'") : "is not null") +
                        " and date " + (year != "" ? ("like '" + year + "%'") : "is not null") + " and type " + (ptype != "" ? (" = '" + ptype + "'") : "is not null") +
                        " and (status " + (statues != "" ? (" = '" + statues + "')") : (" <> '已下达' and status <> '未下达')"));

                if (this.usrKind == "审批员")
                {
                    qStr = "select company,date,amount,type,status,ID from YearPlanning where company " + (cmp != "" ? ("= '" + cmp + "'") : "is not null") +
                        " and date " + (year != "" ? ("like '" + year + "%'") : "is not null") + " and type " + (ptype != "" ? (" = '" + ptype + "'") : "is not null") +
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 8 && e.RowIndex >= 0)
            {
                ...
                运输月计划录入 dlg = new 运输月计划录入(this.dataGridView1.Rows[e.RowIndex].Cells["ID"].Value.ToString(), "上报");
                dlg.ShowDialog();
            }  
        }
            ...
            for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
            {
                if (this.dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "待审")
                    {
                        ...
                        计划审批 dlg = new 计划审批(this.dataGridView1.Rows[i].Cells["ID"].Value.ToString(), "年");
                        dlg.ShowDialog();
                    }
                    else if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "新增")
                    {
                        MessageBox.Show("此年计划尚未提交！", "错误");
                    }
                    else if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "通过")
                    {
                        MessageBox.Show("此年计划已审批通过！", "禁止");
                    }
                }
            ...
            for (int i = 0; i < this.dataGridView1.Rows.Count; i++)
            {
                if (this.dataGridView1.Rows[i].Cells[0].EditedFormattedValue.ToString() == "True")
                {
                    if (this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "通过" || 
                        this.dataGridView1.Rows[i].Cells[7].Value.ToString() == "不通过")
                    {
                        审批单 dlg = new 审批单(this.dataGridView1.Rows[i].Cells["ID"].Value.ToString(), "年");
                        dlg.ShowDialog();
                    }
                    else
                    {
                        MessageBox.Show("该年计划尚未审批！", "提示");
                    }
                }
